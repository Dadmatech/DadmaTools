...
def download_model(model_name: str, cache_dir: str = DEFAULT_CACHE_DIR, process_func: Callable = None,
    ...
    if process_func is not None and not os.path.exists(unzip_dir):
        ...
        process_func(model_file_path, model_info, cache_dir=DEFAULT_CACHE_DIR, unzip_dir=unzip_dir,
...
def download_hf(model_name: str):
    if not os.path.exists('./saved_models'): os.makedirs('saved_models', exist_ok=True)

    if model_name not in MODELS:
        raise ValueError("The model {} do not exist".format(model_name))
    hf_hub_download(repo_id=f"Dadmatech/Vocab", filename='persian.vocabs.json',
                    local_dir=f"./saved_models")
    if model_name == 'kasreh_ezafeh':
        hf_hub_download(repo_id=f"Dadmatech/Kasreh_ezafe", filename='customized-ner.ner.mdl',
                        local_dir=f"./saved_models/{model_name}")
    elif model_name == 'fa_lemmatizer':
        hf_hub_download(repo_id=f"Dadmatech/Lemmatizer", filename='persian_lemmatizer.pt',
                        local_dir=f"./saved_models/{model_name}")
    elif model_name == 'fa_mwt':
        hf_hub_download(repo_id=f"Dadmatech/mwt_expander", filename='persian_mwt_expander.pt',
                        local_dir=f"./saved_models/{model_name}")
    elif model_name == 'postagger':
        hf_hub_download(repo_id=f"Dadmatech/POS", filename='persian.tagger.mdl',
                        local_dir=f"./saved_models/{model_name}")
    elif model_name == 'ner':
        hf_hub_download(repo_id=f"Dadmatech/NER", filename='customized-ner.ner.mdl',
                        local_dir=f"./saved_models/{model_name}")
    elif model_name == 'sent':
        hf_hub_download(repo_id=f"Dadmatech/Sent", filename='persian.sent.mdl',
                        local_dir=f"./saved_models/{model_name}")
    elif model_name == 'fa_tokenizer':
        hf_hub_download(repo_id=f"Dadmatech/tokenizer", filename='persian.tokenizer.mdl',
                        local_dir=f"./saved_models/{model_name}")
    elif model_name == 'spellchecker':
        hf_hub_download(repo_id=f"Dadmatech/Nevise", filename='state_dict_nevise.pt',
                        local_dir=f"./saved_models/{model_name}")
        hf_hub_download(repo_id=f"Dadmatech/Nevise", filename='vocab.pkl',
                        local_dir=f"./saved_models/{model_name}")
        hf_hub_download(repo_id=f"Dadmatech/Nevise", filename='vocab.txt',
                        local_dir=f"./saved_models/{model_name}")
        hf_hub_download(repo_id=f"Dadmatech/Nevise", filename='config.json',
                        local_dir=f"./saved_models/{model_name}")
...
def _unzip_process_func(tmp_file_path: str, meta_info: dict, unzip_dir: str, cache_dir: str = DEFAULT_CACHE_DIR,
